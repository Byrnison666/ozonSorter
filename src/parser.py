...
class ExcelParser:
    # Обязательные колонки для распознавания шапки. Только то, что есть во всех
    # вариантах отчёта Ozon и реально нужно: номер отправления (id клиента + ключ)
    # и ячейка. Тип/Статус Ozon в новом формате (3 колонки) убрал — они опциональны
